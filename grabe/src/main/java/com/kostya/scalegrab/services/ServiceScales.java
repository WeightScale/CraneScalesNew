package com.kostya.scalegrab.services;
...
public class ServiceScales extends Service {
    ...
    public static final String ACTION_CONNECT_BOOTLOADER = "com.konst.simple_scale.ACTION_CONNECT_BOOTLOADER";
    ...
    public static final String TAG = ServiceScales.class.getName();
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
            ...
            if (action != null){
                switch (action){
                    case ACTION_CONNECT_SCALES:
                        ...
                        break;
                    case ACTION_CONNECT_BOOTLOADER:
                        connectBoot(intent);
                        break;
        ...
        sendNotification(getString(R.string.app_name),getString(R.string.app_name),"Программа \"Весы крановые\" ");
    ...
    private void connectBoot(Intent intent){
        Bundle bundle = intent.getBundleExtra(EXTRA_BUNDLE);
        if (bundle != null){
            String version = bundle.getString(EXTRA_VERSION);
            String device = bundle.getString(EXTRA_DEVICE);
            try {
                BootModule.create(getApplicationContext(), version, device, interfaceCallbackScales);
                Toast.makeText(getBaseContext(), R.string.bluetooth_off, Toast.LENGTH_SHORT).show();
            } catch (Exception e) {
                Toast.makeText(getBaseContext(), e.getMessage(), Toast.LENGTH_LONG).show();
                stopSelf();
    ...
    public void sendNotification(CharSequence Ticker, CharSequence Title, CharSequence Text) {
    ...
    public void setNotifyContentText(CharSequence text){
        builder.setContentText(text);
        notificationManager.notify(DEFAULT_NOTIFICATION_ID, builder.build());
    }

    public void setNotifyProgress(CharSequence text, boolean f){
        if (f){
