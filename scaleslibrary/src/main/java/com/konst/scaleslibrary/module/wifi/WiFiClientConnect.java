 ...
 */
class WiFiClientConnect extends Thread implements InterfaceBluetoothClient {
    ...
    private static final String TAG = WiFiClientConnect.class.getName();

    public WiFiClientConnect(Socket socket, BluetoothHandler handler) {
        mmSocket = socket;
    ...
    @Override
    public void run() {
        handler.obtainMessage(BluetoothHandler.MSG.CONNECT.ordinal()).sendToTarget();
        try {
            while (!isInterrupted()) {
            ...
            if(!isTerminate)
                handler.obtainMessage(BluetoothHandler.MSG.DISCONNECT.ordinal()).sendToTarget();
            /*else
